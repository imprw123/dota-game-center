    ...
    <div class="img-box">
      ...
      <div class="action">
        <span
          ...
          @click="AddWebCartGoods(item.Goods_id)"
        >+ 购物车</span>
        <span
          class="send"
          v-if="item.Class_id != 583"
          @click="sendParentCartGods(item.Goods_id,item.Goods_imgPath,item.Goods_disName)"
        >赠送</span>
        <span
          class="ljgm"
  ...
  methods: {
    AddWebCartGoods(goodsid) {
      ...
      this.$axios(
        ...
        }?beGivenUserId=${0}&goodsId=${goodsid}&count=${1}`
      )
        .then(res => {
          // console.log(res);
          if (res.code == 0) {
